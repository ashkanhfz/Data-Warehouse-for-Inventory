...
CREATE OR ALTER PROCEDURE First_Fill_trn_fact_inventory
AS 
BEGIN
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'First_Fill_trn_fact_inventory'  , GETDATE() , 'start run procedure'   , ' ' , 0)
    TRUNCATE TABLE trn_fact_inventory;
   ...
   SELECT @current_day = 
    CASE 
        WHEN (SELECT MIN([time]) FROM [SA].[dbo].[SA_Warehouse_Outlet]) < (SELECT MIN([time]) FROM [SA].[dbo].[SA_Warehouse_Entrance])
        THEN (SELECT MIN([time]) FROM [SA].[dbo].[SA_Warehouse_Outlet])
        ELSE (SELECT MIN([time]) FROM [SA].[dbo].[SA_Warehouse_Entrance])
    END;
    ...
    BEGIN
			...
			select @number_of_rows = count(*) from [dbo].[tmp1_trn_fact_inventory]
...
select @number_of_rows = count(*) - @number_of_rows from [dbo].[tmp1_trn_fact_inventory]
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'First_Fill_trn_fact_inventory'  , GETDATE() , 'insert outlet data'   , '[dbo].[tmp1_trn_fact_inventory]' , @number_of_rows)
...
select @number_of_rows = count(*) - @number_of_rows from [dbo].[tmp1_trn_fact_inventory]
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'First_Fill_trn_fact_inventory'  , GETDATE() , 'insert entrance data'   , '[dbo].[tmp1_trn_fact_inventory]' , @number_of_rows)
...
select @number_of_rows = count(*) - @number_of_rows from sa.[dbo].[SA_Commuting_time]
	insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
	(  'First_Fill_trn_fact_inventory'  , GETDATE() , concat ( 'insert data of ', convert ( varchar,@current_day ))  , ' DatawareHouse.dbo.trn_fact_inventory' , @number_of_rows)


        SET @current_day = DATEADD(DAY, 1, @current_day);
    END;
	insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
( 'First_Fill_trn_fact_inventory'  , GETDATE() , 'end run procedure'   , ' ' , 0)
END;
...
CREATE OR ALTER PROCEDURE FIRST_FILL_Periodic_Fact_inventory
AS
BEGIN
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'FIRST_FILL_Periodic_Fact_inventory'  , GETDATE() , 'start run procedure'   , ' ' , 0);

    TRUNCATE TABLE DatawareHouse.dbo.periodic_fact_inventory;
    TRUNCATE TABLE DatawareHouse.dbo.tmp_periodic_fact_inventory;
	truncate table tmp_cros_join_dimW_dimP;
    ...
    DECLARE @current_day DATETIME;
	declare @number_of_rows int;
    SELECT @current_day = MIN(DateID), @end_date = MAX(DateID) 
    ...
    BEGIN
		...
		select @number_of_rows = count(*) from  DatawareHouse.dbo.tmp_periodic_fact_inventory
		...
		insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
		(  'FIRST_FILL_Periodic_Fact_inventory'  , GETDATE() , 'insert'   , 'DatawareHouse.dbo.tmp_periodic_fact_inventory' , @number_of_rows)
		select @number_of_rows = count(*) from DatawareHouse.dbo.periodic_fact_inventory
		...
		select @number_of_rows = count(*) - @number_of_rows from DatawareHouse.dbo.periodic_fact_inventory
		insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
	(  'FIRST_FILL_Periodic_Fact_inventory'  , GETDATE() , concat ( 'insert data of ', convert ( varchar,@current_day ))  , 'DatawareHouse.dbo.periodic_fact_inventory' , @number_of_rows)

        TRUNCATE TABLE DatawareHouse.dbo.tmp_periodic_fact_inventory;
        SET @current_day = DATEADD(DAY, 1, @current_day);
    END;
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
( 'FIRST_FILL_Periodic_Fact_inventory'  , GETDATE() , 'end run procedure'   , ' ' , 0)
END;

GO

CREATE OR ALTER PROCEDURE FIRST_FILL_ACC_Fact_inventory
AS
BEGIN
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'FIRST_FILL_ACC_Fact_inventory'  , GETDATE() , 'start run procedure'   , ' ' , 0)
	...
	select @number_of_rows = count(*) from tmp1_acc_fact_inventory
		...
		select @number_of_rows = count(*)- @number_of_rows from tmp1_acc_fact_inventory
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'FIRST_FILL_ACC_Fact_inventory'  , GETDATE() , 'insert'   , 'tmp1_acc_fact_inventory' , @number_of_rows)
        TRUNCATE TABLE tmp2_acc_fact_inventory;
select @number_of_rows = count(*) from tmp2_acc_fact_inventory
...
select @number_of_rows = count(*)- @number_of_rows from tmp2_acc_fact_inventory
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'FIRST_FILL_ACC_Fact_inventory'  , GETDATE() , 'insert'   , 'tmp2_acc_fact_inventory' , @number_of_rows)
select @number_of_rows = count(*) from tmp_acc_fact_inventory
		...
		select @number_of_rows = count(*)- @number_of_rows from tmp_acc_fact_inventory
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'FIRST_FILL_ACC_Fact_inventory'  , GETDATE() , 'insert'   , 'tmp_acc_fact_inventory' , @number_of_rows)

        SET @current_day = DATEADD(DAY, 1, @current_day);
    END;
	select @number_of_rows = count(*) from DatawareHouse.dbo.acc_fact_inventory
    ...
    FROM DatawareHouse.dbo.tmp_acc_fact_inventory;
			select @number_of_rows = count(*)- @number_of_rows from DatawareHouse.dbo.acc_fact_inventory
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'FIRST_FILL_ACC_Fact_inventory'  , GETDATE() , 'insert'   , 'DatawareHouse.dbo.acc_fact_inventory' , @number_of_rows)
...
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
( 'FIRST_FILL_ACC_Fact_inventory'  , GETDATE() , 'end run procedure'   , ' ' , 0)
END;

GO 
CREATE OR ALTER PROCEDURE FIRST_FILL_trn_fact_discount
AS
BEGIN
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'first_fill_dim_company'  , GETDATE() , 'start run procedure'   , ' ' , 0)
    TRUNCATE TABLE trn_fact_discount;

    DECLARE @end_date DATETIME;
    DECLARE @current_day DATETIME;
	declare @number_of_rows int;

    SELECT @current_day = MIN([time]), @end_date = MAX([time]) 
    FROM [SA].[dbo].[SA_Warehouse_Outlet];

    WHILE @current_day <= @end_date
    BEGIN
	select @number_of_rows = count(*) from [dbo].[tmp1_trn_fact_discount]
...
select @number_of_rows = count(*) from [dbo].[tmp1_trn_fact_discount]
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'first_fill_dim_company'  , GETDATE() , 'insert'   , '[dbo].[tmp1_trn_fact_discount]' , @number_of_rows)

select @number_of_rows = count(*) from DatawareHouse.dbo.trn_fact_discount
	...
	select @number_of_rows = count(*)- @number_of_rows from DatawareHouse.dbo.trn_fact_discount
	insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
	(  'first_fill_dim_company'  , GETDATE() , concat ( 'insert data of ', convert ( varchar,@current_day ))  , 'DatawareHouse.dbo.trn_fact_discount' , @number_of_rows)
        ...
        SET @current_day = DATEADD(DAY, 1, @current_day);
		insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
( 'first_fill_dim_company'  , GETDATE() , 'end run procedure'   , ' ' , 0)
    END;
END;


go 
CREATE OR ALTER PROCEDURE FIRST_FILL_Factless_fact_HR
AS
BEGIN
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'FIRST_FILL_Factless_fact_HR'  , GETDATE() , 'start run procedure'   , ' ' , 0)

declare @number_of_rows int;
select @number_of_rows = count(*) from factless_HR


	truncate table factless_HR;
	INSERT INTO factless_HR(EmployeeID1,EmployeeID2)
	select representative_id,presented_id
	from sa.dbo.SA_Representative
select @number_of_rows = count(*) - @number_of_rows from factless_HR
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'FIRST_FILL_Factless_fact_HR'  , GETDATE() , 'insert'   , 'factless_HR' , @number_of_rows)
END;



DROP TABLE IF EXISTS tmp_Periodic_fact_discount;
CREATE TABLE tmp_Periodic_fact_discount (
    DiscountTypeID INT,
    DateID DATE,
    customerID INT,
    -----------------------------
...
GO 
CREATE OR ALTER PROCEDURE FIRST_FILL_Periodic_fact_discount
AS
BEGIN
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'FIRST_FILL_Periodic_fact_discount'  , GETDATE() , 'start run procedure'   , ' ' , 0)
    TRUNCATE TABLE DatawareHouse.dbo.periodic_fact_discount;
    ...
    BEGIN
		...
		declare @number_of_rows int;
select @number_of_rows = count(*) from DatawareHouse.dbo.Periodic_fact_discount
		...
		select @number_of_rows = count(*) - @number_of_rows from  DatawareHouse.dbo.Periodic_fact_discount
	insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
	(  'FIRST_FILL_Periodic_fact_discount'  , GETDATE() , concat ( 'insert data of ', convert ( varchar,@current_day ))  , ' DatawareHouse.dbo.Periodic_fact_discount' , @number_of_rows)

        TRUNCATE TABLE DatawareHouse.dbo.tmp_periodic_fact_discount;
        SET @current_day = DATEADD(DAY, 1, @current_day);
    END;
	insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
	( 'FIRST_FILL_Periodic_fact_discount'  , GETDATE() , 'end run procedure'   , ' ' , 0)
END;
GO

Go 
CREATE OR ALTER PROCEDURE FIRST_FILL_FACTLESS_FACT_discount
AS
BEGIN
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'FIRST_FILL_FACTLESS_FACT_discount'  , GETDATE() , 'start run procedure'   , ' ' , 0)
declare @number_of_rows int;
	...
	from SA.dbo.SA_Discount_code
	select @number_of_rows = count(*) from factless_fact_discount
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'FIRST_FILL_FACTLESS_FACT_discount'  , GETDATE() , 'insert'   , 'factless_fact_discount' , @number_of_rows)
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
( 'FIRST_FILL_FACTLESS_FACT_discount'  , GETDATE() , 'end run procedure'   , ' ' , 0)
END;


GO
CREATE OR ALTER PROCEDURE FIRST_FILL_ACC_Fact_discount
AS
BEGIN
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'FIRST_FILL_ACC_Fact_discount'  , GETDATE() , 'start run procedure'   , ' ' , 0)
    ...
    END;
	select @number_of_rows = count(*) from DatawareHouse.dbo.acc_fact_discount
    ...
    SELECT
        ...
        NumberOfUses
    FROM tmp_acc_fact_discount;
	select @number_of_rows = count(*) - @number_of_rows from DatawareHouse.dbo.acc_fact_discount
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'FIRST_FILL_ACC_Fact_discount'  , GETDATE() , ' insert'   , 'DatawareHouse.dbo.acc_fact_discount' , @number_of_rows)
...
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
( 'FIRST_FILL_ACC_Fact_discount'  , GETDATE() , 'end run procedure'   , ' ' , 0)
END;


Go 
CREATE OR ALTER PROCEDURE FIRST_FILL_FACT_INVENTORY
AS 
begin
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'FIRST_FILL_FACT_INVENTORY'  , GETDATE() , 'start run procedure'   , ' ' , 0)
declare @number_of_rows int;
...
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'FIRST_FILL_FACT_INVENTORY'  , GETDATE() , 'insert'   , 'factless_fact_inventory' , @number_of_rows)
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
( 'FIRST_FILL_FACT_INVENTORY'  , GETDATE() , 'end run procedure'   , ' ' , 0)
end;


GO

CREATE OR ALTER PROCEDURE First_Fill_trn_fact_HR
AS
BEGIN
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'First_Fill_trn_fact_HR'  , GETDATE() , 'start run procedure'   , ' ' , 0)

	TRUNCATE TABLE DatawareHouse.dbo.trn_fact_HR;
	...
	SELECT @current_day =
		CASE
		WHEN (SELECT MIN(date) FROM [SA].[dbo].[SA_Rollcall]) < (SELECT MIN(date) FROM [SA].[dbo].[SA_Commuting_time])
		THEN (SELECT MIN(date) FROM [SA].[dbo].SA_Rollcall)
		ELSE (SELECT MIN(date) FROM [SA].[dbo].SA_Commuting_time)
	END;
	...
	WHILE @current_day <= @end_date
		...
		select @number_of_rows = count(*) from  DatawareHouse.dbo.trn_fact_HR
...
select @number_of_rows = count(*) - @number_of_rows from  DatawareHouse.dbo.trn_fact_HR
	insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
	(  'First_Fill_trn_fact_HR'  , GETDATE() , concat ( 'insert data of ', convert ( varchar,@current_day ))  , ' DatawareHouse.dbo.trn_fact_HR' , @number_of_rows)


		SET @current_day = DATEADD(DAY, 1, @current_day);
	END;
	insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
( 'First_Fill_trn_fact_HR'  , GETDATE() , 'end run procedure'   , ' ' , 0)
END;
...
GO 
CREATE OR ALTER PROCEDURE FIRST_FILL_Periodic_fact_HR
AS
BEGIN
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'FIRST_FILL_Periodic_fact_HR'  , GETDATE() , 'start run procedure'   , ' ' , 0)
    TRUNCATE TABLE DatawareHouse.dbo.monthly_fact_HR;

    DECLARE @min_date DATE;
    DECLARE @max_date DATE;
	declare @number_of_rows int;

    SELECT @min_date = MIN(DateID), @max_date = MAX(DateID) 
    FROM [DatawareHouse].dbo.trn_fact_HR;

    WHILE @min_date <= @max_date
    BEGIN
	select @number_of_rows = count(*)from DatawareHouse.dbo.monthly_fact_HR
		...
		select @number_of_rows = count(*) - @number_of_rows from DatawareHouse.dbo.monthly_fact_HR
	insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
	(  'Fill_SA_Cummuting_time'  , GETDATE() , concat ( 'insert data of ', convert ( varchar,@min_date ))  , 'DatawareHouse.dbo.monthly_fact_HR' , @number_of_rows)

        SET @min_date = DATEADD(month, 1, @min_date);
    END;
	insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
( 'FIRST_FILL_Periodic_fact_HR'  , GETDATE() , 'end run procedure'   , ' ' , 0)
END;
GO


GO
CREATE OR ALTER PROCEDURE FIRST_FILL_ACC_Fact_HR
AS
BEGIN
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'FIRST_FILL_ACC_Fact_HR'  , GETDATE() , 'start run procedure'   , ' ' , 0)
    ...
    END;
	select @number_of_rows = count(*) from acc_fact_HR
	Truncate Table acc_fact_HR;
    INSERT INTO acc_fact_HR (
		...
		select @number_of_rows = count(*) - @number_of_rows from acc_fact_HR
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
(  'FIRST_FILL_ACC_Fact_HR'  , GETDATE() , 'insert'   , 'acc_fact_HR' , @number_of_rows)
    ...
    VALUES (@end_date);
insert into DatawareHouse.dbo.Log (procedure_name,date,description , table_name ,number_of_row_inserted) values
( 'FIRST_FILL_ACC_Fact_HR'  , GETDATE() , 'end run procedure'   , ' ' , 0)
END;
